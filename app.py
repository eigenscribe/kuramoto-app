...
tab1, tab2, tab3, tab4 = st.tabs(["Network", "Distributions", "Animation", "JSON Import"])
...
########################
# TAB 4: JSON IMPORT TAB
########################
with tab4:
    # Update current tab in session state
    st.session_state.current_tab = "JSON Import"
    st.markdown("<h2 class='gradient_text2'>JSON Parameter Import</h2>", unsafe_allow_html=True)
    
    # Display explanation and example
    st.markdown("""
    <div class='section'>
        <p>Import parameters for the Kuramoto simulation using JSON format. This allows you to easily save and load complex configurations, including custom adjacency matrices.</p>
        <p>Paste your JSON configuration in the text area below and click "Import Parameters" to apply the settings.</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Create example JSON with collapsible section
    with st.expander("Show Example JSON Format", expanded=True):
        example_json = {
            "n_oscillators": 10,
            "coupling_strength": 1.0,
            "network_type": "All-to-All", 
        ...
        st.markdown("""
        <div class='section'>
            <h3 class='gradient_text1'>Parameter Details</h3>
            <ul>
                <li><b>n_oscillators</b>: Number of oscillators (integer)</li>
                <li><b>coupling_strength</b>: The K parameter that determines coupling strength (float)</li>
                <li><b>network_type</b>: One of "All-to-All", "Nearest Neighbor", "Random", "Custom Adjacency Matrix"</li>
                <li><b>simulation_time</b>: Total time to simulate (float)</li>
                <li><b>time_step</b>: Time step for numerical integration (float)</li>
                <li><b>random_seed</b>: Seed for random number generation (integer)</li>
                <li><b>frequency_distribution</b>: One of "Normal", "Uniform", "Custom", "Golden Ratio"</li>
                <li><b>frequency_parameters</b>: Parameters specific to the chosen distribution</li>
                <li><b>adjacency_matrix</b>: (Optional) 2D array representing network connectivity</li>
            </ul>
            <p>If you include an adjacency matrix, the network_type will automatically be set to "Custom Adjacency Matrix" regardless of what you specify.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    # Display a secondary example for adjacency matrix
    with st.expander("Advanced Example: Custom Adjacency Matrix", expanded=False):
        st.markdown("""
        <div class='section'>
            <p>Here's an example of a small-world network configuration with 10 oscillators:</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Generate a sample small-world network (this is just for display)
        n = 10
        # Create a ring lattice
        sample_matrix = np.zeros((n, n))
        for i in range(n):
            # Connect to 2 neighbors on each side
            for j in range(1, 3):
        ...
        # Display the example
        st.code(json.dumps(complex_example, indent=4), language="json")
        
        st.markdown("""
        <div class='section'>
            <p>Click the button below to use this example:</p>
        </div>
        """, unsafe_allow_html=True)
