...
########################
# TAB 6: MACHINE LEARNING TAB
########################
with tab6:
    # Update current tab in session state
    st.session_state.current_tab = "Machine Learning"
    st.markdown("<h2 class='gradient_text2'>Machine Learning</h2>", unsafe_allow_html=True)
    
    st.markdown("""
    <div style='background-color: rgba(0,0,0,0.3); padding: 15px; border-radius: 10px; margin-bottom: 20px;'>
        <p>Create, manage and analyze datasets for machine learning applications:</p>
        <ul>
            <li>Generate parameter sweeps for training neural networks</li>
            <li>Study critical coupling phenomena</li>
            <li>Compare different network topologies</li>
            <li>Export datasets in formats suitable for ML frameworks</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    with ml_tab3:
                            ...
                            if any(f['name'] == 'coupling_strength' for f in dataset_details['features']) and \
                               any(f['name'] == 'order_parameter_timeseries' for f in dataset_details['features']):
                                ...
                                with st.spinner("Generating visualization..."):
                                    ...
                                    ax1.scatter(k_sorted, r_final_sorted, c=k_sorted, cmap='viridis', s=50)
                                    ax1.plot(k_sorted, r_final_sorted, 'k--', alpha=0.5)
                                    ax1.set_xlabel('Coupling Strength (K)', fontsize=12)
                                    ax1.set_ylabel('Final Order Parameter (r)', fontsize=12)
                                    ax1.set_title('Phase Transition to Synchronization', fontsize=14)
                                    ...
                                    # 2. Plot time series for various K values
                                    cmap = plt.cm.viridis
                                    for i, idx in enumerate(sort_idx):
                                        color = cmap(i / len(sort_idx))
                                        ax2.plot(r_timeseries[idx], color=color, alpha=0.7, 
                                               label=f'K={k_values[idx]:.2f}' if i % 5 == 0 else "")
                                    
                                    ax2.set_xlabel('Time Step', fontsize=12)
                                    ax2.set_ylabel('Order Parameter (r)', fontsize=12)
                                    ax2.set_title('Synchronization Dynamics for Different K Values', fontsize=14)
                                    ax2.grid(True, alpha=0.3)
                                    
                                    # Add colorbar to indicate K values
                                    from matplotlib.cm import ScalarMappable
                                    sm = ScalarMappable(cmap=cmap, norm=plt.Normalize(k_sorted.min(), k_sorted.max()))
                                    sm.set_array([])
                                    cbar = fig.colorbar(sm, ax=ax2)
                                    cbar.set_label('Coupling Strength (K)')
                                    
                                    # Add a custom legend with fewer items
                                    ...
                                    # Calculate theoretical critical coupling
                                    # For Kuramoto with normal distribution, it's approximately 2*std/π
                                    theoretical_k = 2 * 0.5 / np.pi  # Using standard freq_std=0.5
                                    ...
                                    <div style='background-color: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px;'>
                                        ...
                                        <p><b>Simulations:</b> {len(dataset_details['simulations'])}</p>
                                        <p><b>K Range:</b> {k_sorted.min():.4f} to {k_sorted.max():.4f}</p>
                                        <p><b>Estimated Critical K:</b> {critical_k:.4f}</p>
                                        <p><b>Theoretical Critical K:</b> {theoretical_k:.4f}</p>
                                    </div>
                                    """, unsafe_allow_html=True)
                            else:
        ...
        # Machine learning resources section
        st.markdown("<h3 class='gradient_text1'>Machine Learning Resources</h3>", unsafe_allow_html=True)
        
        st.markdown("""
        <div style='background-color: rgba(0,0,0,0.2); padding: 10px; border-radius: 5px;'>
            <p><b>Resources for Kuramoto Model in Machine Learning:</b></p>
            <ul>
                <li><a href="https://arxiv.org/abs/1909.12790" target="_blank">Learning the Ising Model with Generative Neural Networks</a></li>
                <li><a href="https://arxiv.org/abs/2006.02619" target="_blank">Lagrangian Neural Networks</a></li>
                <li><a href="https://arxiv.org/abs/2010.03957" target="_blank">Learning Dynamical Systems from Partial Observations</a></li>
                <li><a href="https://arxiv.org/abs/1712.09707" target="_blank">Neural Ordinary Differential Equations</a></li>
            </ul>
        </div>
        """, unsafe_allow_html=True)